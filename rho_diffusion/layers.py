...
"""
Part of the code in this file is adopted from OpenAI's Guided Diffusion implementation:
...
@registry.register_layer("RMSNorm")
class RMSNorm(nn.Module):
    """
    Implements the RMSNorm layer.

    The key advantage of this normalization layer over BatchNorm is that it does
    not require a complete batch, and by excluding the mean, makes it that much
    faster.
...
@registry.register_layer("GroupNorm32")
class GroupNorm32(nn.GroupNorm):
    def forward(self, x):
        return super().forward(x.float()).type(x.dtype)


@registry.register_layer("conv_nd")
...
@registry.register_layer("avg_pool_nd")
