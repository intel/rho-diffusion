...
"""
Ported from OpenAI's Guided Diffusion:
...
class GaussianDiffusionPipeline(AbstractDiffusionPipeline):
    """
    ...
    def __init__(
        ...
        if diffusion_defaults['use_kl']:
            self.loss_type = LossType.RESCALED_KL
        elif diffusion_defaults['rescale_learned_sigmas']:
            self.loss_type = LossType.RESCALED_MSE
        else:
            self.loss_type = LossType.MSE

        if diffusion_defaults['predict_xstart'] is True:
        ...
        if diffusion_defaults['learn_sigma'] is True:
            ...
            if diffusion_defaults['sigma_small'] is True:
                self.model_var_type = ModelVarType.FIXED_SMALL
            else:
                self.model_var_type = ModelVarType.FIXED_LARGE

        self.rescale_timesteps = diffusion_defaults['rescale_timesteps']

        # Use float64 for accuracy.
        betas = get_named_beta_schedule(diffusion_defaults['noise_schedule'], diffusion_defaults['diffusion_steps'])
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    ...
                    terms["vb"] *= self.timesteps / 1000.0
    ...
    def _prior_bpd(self, x_start):
        ...
        t = torch.tensor([self.timesteps - 1] * batch_size, device=x_start.device)
